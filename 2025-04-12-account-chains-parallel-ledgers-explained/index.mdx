...
**Less Contention = Low Latency:** Because each user writes their own chain, two users transacting at the same time *usually don’t slow each other down*. Unless they are transacting with each other, they’re basically operating in parallel universes that occasionally intersect. This leads to very low latency. For instance, Atto’s network (which uses account-chains) achieves confirmation times under <Metric name="network.confirmation-time.ms.seven-day-p50" /> milliseconds in many cases – much faster than waiting for a Bitcoin 10-minute block. In general, removing the global bottleneck means the network can handle more transactions per second, limited mostly by network bandwidth rather than strict protocol delays.
