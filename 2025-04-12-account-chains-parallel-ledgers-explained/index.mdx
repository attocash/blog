...
**Less Contention = Low Latency:** Because each user writes their own chain, two users transacting at the same time *usually don’t slow each other down*. Unless they are transacting with each other, they’re basically operating in parallel universes that occasionally intersect. This leads to very low latency. Atto's network currently confirms typical transactions in about <a href="/metrics#confirmation-speed"><Metric name="network.confirmation-time.ms.seven-day-p50" precision={0} suffix=" ms" /></a>, much faster than waiting for a Bitcoin 10-minute block. In general, removing the global bottleneck means the network can handle more transactions per second, limited mostly by network bandwidth rather than strict protocol delays.
