...
Python’s async model, to publishing a usable API client. Hopefully, this helps
...
At the time, there were no API wrappers available for Atto. The only tools available were browser-based:  
...
After poking around the [Atto Wallet org](https://github.com/attocash/) on
...
These are only needed for human-readable addresses, and the API doesn't want them. So I
just sliced them off:
